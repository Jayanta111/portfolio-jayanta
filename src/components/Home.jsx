...
const RESUME_FILE = {
  ...
  image: "/resume-preview.png", // optional preview image
...
function Home() {
  ...
  return (
    <section className="p-6 flex flex-col items-center min-h-screen mt-10">
      {/* Root Folder Grid */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full max-w-6xl mt-10">
        {rootFolder.children.map((item) => (
          <div
            key={item.id}
            className="flex flex-col items-center p-2 rounded-lg cursor-pointer hover:scale-105 transition-transform"
      ...
      {windows.map((folder) => (
        <motion.div
          ...
          className="fixed top-20 left-20 w-80 z-50 bg-gray-100 dark:bg-gray-800 shadow-xl rounded-lg overflow-hidden"
        >
          {/* MacOS-style Header */}
            ...
            {folder.children?.map((item) => (
              ...
              >
                <img
                  ...
                  alt={item.name}
                />
                <p className="text-sm text-center truncate">{item.name}</p>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      {selectedFile && (
        ...
        >
          <div className="bg-gray-200 dark:bg-gray-800 p-6 rounded-lg w-full max-w-lg max-h-[80vh] overflow-y-auto shadow-xl">
            {/* Header */}
